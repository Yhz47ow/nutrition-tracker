...
本项目使用 Capacitor 8 同时封装 iOS 和 Android。Web 资源在构建时复制进原生安装包，不依赖 GitHub Pages，离线可启动；饮食、训练、历史、同步和音乐界面继续共用现有 HTML/CSS/JavaScript。

选择 Capacitor 而不是 Android TWA 的原因：组间倒计时需要 `AlarmManager`、前台服务和 `UNUserNotificationCenter`，iOS Apple Music 需要 MusicKit。两端使用同一种桥接模型也能减少后续维护分叉。
...
- iOS MusicKit 授权、单曲播放、播放/暂停、上一首、下一首和曲目信息。
- PWA/Web 环境保持原 Service Worker 通知和 Web 播放降级。
- Android release keystore 配置入口和三端品牌资源。

尚需在发布机器完成：Xcode/Android Studio 编译、模拟器与真机矩阵测试、Apple Developer Team 配置、MusicKit Capability 校验、Android release keystore 和商店签名。没有签名资料时无法生成可提交的 IPA 或签名 AAB。
...
| `capacitor.config.json` | App ID、Web 目录、状态栏、启动页和外部音乐域名 |
...
| `ios/App/CapApp-SPM/Sources/CapApp-SPM/AppleMusicPlugin.swift` | iOS MusicKit 桥接 |
| `ios/App/App/AppDelegate.swift` | iOS 插件注册和音频会话 |
...
4. 添加并确认 MusicKit Capability；仓库中的 entitlement 为 `com.apple.developer.musickit`。
5. 在已登录 Apple Music 且有有效订阅的真机上测试。模拟器不能代表完整 MusicKit 行为。
6. 选择模拟器或真机运行。
...
iOS 原生 Apple Music 流程通过 MusicKit 完成。用户在音乐设置中选择 Apple Music 并填入单曲链接或歌曲 ID，首次播放会请求媒体资料库授权。单曲 ID 可交给原生播放器；专辑和歌单继续显示 Apple 官方嵌入播放器。

iOS 不提供第三方 App 可用的独立系统音量写入 API，因此音量滑块在原生 Apple Music 模式禁用，用户使用侧边音量键或控制中心。绕过系统限制的私有 API 不能用于 App Store。

Android 上 Apple Music、网易云、QQ音乐和小宇宙继续使用 Web 嵌入或“在平台打开”。Apple Music Android SDK 需要开发者令牌和服务端签名，不符合当前纯前端、无后端约束，因此没有伪造一个不安全的原生实现。平台限制详见 `MUSIC_INTEGRATION.md`。
...
提交前必须配置唯一 Bundle ID、Apple Developer Team、分发证书、描述文件、MusicKit Capability、隐私说明、截图和 App Store Connect 元数据。Apple Music 必须用真实账号和真机验收。
...
当前机器只有 Xcode Command Line Tools 和 Swift，缺少完整 Xcode/iOS Simulator；同时缺少 JDK、Android SDK 和 Android Studio。因此本次可以验证 Web 测试、Capacitor 同步、XML/plist、资源与 MusicKit API 形态，但不能声称已在模拟器/真机运行，也不能在这里产出 APK/AAB/IPA。完整工具链安装和开发者签名是剩余发布前置条件。
