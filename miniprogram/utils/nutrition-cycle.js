...
const DEFAULT_TAPER=Object.freeze({startCalories:2000,stageDays:7,stageCount:4,carbDrop:20});
...
function normalizeTaperSettings(settings){
  ...
  return {
    ...
    stageDays:Math.max(1,Math.round(number(source.stageDays,DEFAULT_TAPER.stageDays))),
    stageCount:Math.max(1,Math.round(number(source.stageCount,DEFAULT_TAPER.stageCount))),
...
function generate532Taper(settings){
  ...
  if(cycleLength>31)return {error:'计划总天数不能超过31天',config,days:[]};
  ...
  if(lastCarbs<=0)return {error:'降碳幅度过大，请减少阶段数或每阶段降幅',config,days:[]};
  const days=[];
  for(let stage=0;stage<config.stageCount;stage+=1){
    ...
    for(let day=0;day<config.stageDays;day+=1){
      days.push(normalizeDay({name:`第${stage+1}阶段 · 第${day+1}天`,calories,carbs:ratios.carbs,protein:ratios.protein,fat:ratios.fat},days.length));
    }
  }
...
module.exports={DEFAULT_RATIO,DEFAULT_TAPER,macroTargets,ratiosFromGrams,normalizeTaperSettings,generate532Taper,normalizeDay,createDefaultCycle,normalizeCycle,ratioTotal,cycleDay,normalizeCycleState,resolve};
