...
应用启动时读取系统 `theme`，切换系统外观时通过 `wx.onThemeChange` 更新当前页面、导航栏和 TabBar。存储版本已升级到 v6，旧数据会自动初始化收藏、套餐、训练计划和训练日/休息日营养目标。
...
当前已使用正式 AppID 完成微信开发者工具预览编译，主包约 208KB。发布前仍需在真实手机完成以下验收：
