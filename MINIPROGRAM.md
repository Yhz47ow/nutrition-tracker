...
- 搜索：71 种中国常见食物、自定义食物和可选 OpenFoodFacts 联网搜索。
...
- 设置：身体资料与基础代谢、营养目标、主题、微信聊天 JSON 导入导出和数据清除；入口位于“记录”页右上角。
...
应用启动时读取系统 `theme`，切换系统外观时通过 `wx.onThemeChange` 更新当前页面、导航栏和 TabBar。存储版本已升级到 v3，旧版默认浅色设置会迁移为“跟随系统”。
...
本地记录本身不需要网络。OpenFoodFacts 联网食品搜索需要在微信公众平台配置 `request` 合法域名：

```text
https://world.openfoodfacts.org
```

开发期间可以在微信开发者工具的“详情 > 本地设置”暂时关闭“校验合法域名”。正式体验版和发布版必须在公众平台完成域名配置；如果账号类型或平台策略不允许配置第三方域名，中国常见食物和自定义食物仍可正常使用，但联网查询不可用。

### 中国食物数据源说明

《中国居民膳食指南》提供饮食结构建议，并不是可直接查询每 100g 营养素的开放食物 API。常用营养数据来源《中国食物成分表》是另一套专业数据，当前项目没有其官方授权接口，因此不会抓取网页或把估算数据冒充官方数据。

现阶段采用“中国常见食物本地库 + 用户自定义食物 + 可选国际联网食品”的组合。若后续取得《中国食物成分表》或其他中国营养数据库的合法授权、API 文档和使用条款，可以在不改变饮食记录模型的情况下替换联网数据源。
