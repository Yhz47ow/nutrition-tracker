...
- 搜索：120 种本地中国常见食物，支持膳食类别、汉字和拼音首字母筛选。
- 食物库：内置食物、自定义食物、收藏和自定义套餐；均可离线使用。
- 自定义食物与套餐：按每份营养录入食物；套餐可组合多种食物并一次拆分记录。
...
当前已使用正式 AppID 完成微信开发者工具预览编译，主包约 187KB。发布前仍需在真实手机完成以下验收：
