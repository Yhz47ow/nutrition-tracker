...
- 食物库：213 种本地中国常见基础食材，集中提供膳食类别、汉字和拼音首字母筛选、收藏、自定义食物和套餐。
- 家常菜：24 道本地估算配方，展示整菜营养，支持按建议份量缩放和逐项修改食材克重。
- 自定义食物与套餐：按每份营养录入食物；热量支持 kcal（千卡/大卡）与 kJ（千焦）自动换算；套餐可组合多种食物并一次拆分记录。
- 食物记录：克数快捷调节、餐次、照片、营养预览和推荐份量。
- 训练：六大肌群动作库、自定义动作、多动作、多组、重量/次数快捷调节和每动作休息时间。
- 训练计划：月历手动标记训练日或休息日，自由填写计划名称、训练内容和备注。
- 饮食计划：分别保存训练日、休息日和未安排日目标，并按训练月历自动套用。
- 周期饮食：自定义 1-31 天循环、每天热量与碳蛋脂比例，并支持今日临时覆盖。
- 组间休息：环形倒计时、暂停、跳过、增加 10 秒、结束提示音和震动。
- 历史：饮食日期记录、训练月历、每组详情和 Canvas 动作重量趋势。
- 设置：身体资料与基础代谢、营养目标、主题、微信聊天 JSON 导入导出和数据清除；入口位于首页右上角。
...
| `customFoods` | 用户自定义食物 |
| `mealTemplates` | 用户自定义套餐及组成食物快照 |
| `favoriteFoods` | 收藏的内置或自定义食物 ID |
| `userSettings` | 营养目标、主题偏好和身体资料 |
| `workoutHistory` | 已结束训练 |
| `workoutPlans` | 按日期保存的手动训练日与休息日计划 |
...
应用启动时读取系统 `theme`，切换系统外观时通过 `wx.onThemeChange` 更新当前页面、导航栏和 TabBar。存储版本已升级到 v7，旧数据会自动初始化收藏、套餐、训练计划、双目标和周期饮食设置。

## 身体资料与基础代谢

设置页本地保存生理性别、年龄、身高和体重，并使用 Mifflin-St Jeor 公式估算基础代谢量。基础代谢只表示静息状态下维持生命活动的估算消耗，不等同于包含日常活动和训练的每日总热量需求。身体资料会随 JSON 备份导出，但不会上传服务器。

当前先在设置页展示结果，首页是否增加代谢卡片、替换“总重量”指标或与每日热量目标联动，待确认产品位置后再调整。
...
《中国居民膳食指南》提供饮食结构建议，并不是可直接查询每 100g 营养素的开放食物 API。常用营养数据来源《中国食物成分表》是另一套专业数据，当前项目没有其官方授权接口，因此不会抓取网页或把估算数据冒充官方数据。

现阶段采用“中国常见食物本地库 + 用户自定义食物和套餐”的组合，不进行任何联网查询。每条食物包含膳食宝塔类别、默认份量、可食部、热量、蛋白质、脂肪、碳水、膳食纤维和钠；详细维护规则见 `miniprogram/data/FOOD_DATA_NOTES.md`。若后续取得《中国食物成分表》或其他中国营养数据库的合法授权，可以在不改变饮食记录模型的情况下批量校订本地数据。
...
当前已使用正式 AppID 完成微信开发者工具预览编译，主包约 236KB。发布前仍需在真实手机完成以下验收：

1. 使用汉字和拼音首字母搜索，按类别筛选并收藏食物。
2. 新增、编辑、删除自定义食物并记录四种餐次。
3. 创建、编辑、删除套餐并确认一次记录会生成多个正常食物记录。
4. 拍照、预览、删除食物照片。
5. JSON 导出后清除数据，再导入恢复，核对收藏、自建食物和套餐。
6. 新建自定义动作并按肌群筛选。
7. 完成一组，测试暂停、跳过、增加 10 秒和返回前台校准。
8. 结束训练，检查月历、组详情和趋势。

## 上传体验版

1. 在微信开发者工具确认当前 AppID 和真机测试结果。
2. 点击右上角“上传”，版本号填写 `2.2.0`，备注填写“本地中国食物库、收藏与自定义套餐”。
